    ...
    <el-container>
      ...
      <el-header style="font-size: 12px; border-bottom: 1px solid #ccc; line-height: 60px; display: flex">
        ...
        <el-dropdown style="width: 70px; cursor: pointer">
          <span>Charon.Wang</span><i class="el-icon-arrow-down" style="margin-left: 5px"></i>
...
export default {
  ...
  methods: {
    ...
    //分页查询
    load() {
      request.get("http://localhost:9001/user/page", {
        params: {
    ...
    //新增->保存
    save() {
      request.post("http://localhost:9001/user", this.form).then(res => {
        if (res) {
    ...
    //删除
    del(id) {
      request.delete("http://localhost:9001/user/delete/" + id).then(res => {
        if (res) {
